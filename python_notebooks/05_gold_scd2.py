...
total = spark.sql(f"SELECT COUNT(*) AS cnt FROM {GOLD_SCHEMA}.dim_restaurant").collect()[0]["cnt"]
current = spark.sql(f"SELECT COUNT(*) AS cnt FROM {GOLD_SCHEMA}.dim_restaurant WHERE is_current = True").collect()[0]["cnt"]
...
bad_expired = spark.sql(f"""
    SELECT * FROM {GOLD_SCHEMA}.dim_restaurant
