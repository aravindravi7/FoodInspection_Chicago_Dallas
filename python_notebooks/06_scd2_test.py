...
# spark.sql(f"DROP TABLE IF EXISTS {GOLD_SCHEMA}.dim_restaurant")
...
total_before = spark.sql(f"SELECT COUNT(*) AS cnt FROM {GOLD_SCHEMA}.dim_restaurant").collect()[0]["cnt"]
...
total_after = spark.sql(f"SELECT COUNT(*) AS cnt FROM {GOLD_SCHEMA}.dim_restaurant").collect()[0]["cnt"]
current_after = spark.sql(f"SELECT COUNT(*) AS cnt FROM {GOLD_SCHEMA}.dim_restaurant WHERE is_current = True").collect()[0]["cnt"]
...
bad_expired = spark.sql(f"""
    SELECT COUNT(*) AS cnt FROM {GOLD_SCHEMA}.dim_restaurant
...
display(spark.sql(f"DESCRIBE HISTORY {GOLD_SCHEMA}.dim_restaurant"))
...
total_no_change = spark.sql(f"SELECT COUNT(*) AS cnt FROM {GOLD_SCHEMA}.dim_restaurant").collect()[0]["cnt"]
expired_no_change = spark.sql(f"SELECT COUNT(*) AS cnt FROM {GOLD_SCHEMA}.dim_restaurant WHERE is_current = False").collect()[0]["cnt"]
